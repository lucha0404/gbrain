  ...
  try {
    ...
    if (phases.includes('propose_takes') ||
      ...
      if (engine) {
        ...
        const calibrationModels = calibrationConfig as {
          expansion_model?: string;
          models?: { brainstorm?: { judge?: string } };
        };
        const calibrationExpansionModel = calibrationModels.expansion_model
          ?? await engine.getConfig('expansion_model')
          ?? undefined;
        const calibrationJudgeModel = calibrationModels.models?.brainstorm?.judge
          ?? await engine.getConfig('models.brainstorm.judge')
          ?? calibrationExpansionModel;
